...
interface FlipsPanelProps {
  ...
  onUpdate: (configs: FlipConfig[]) => void;
}
...
export default function FlipsPanel({ flipConfigs, onUpdate }: FlipsPanelProps) {
  const addFlip = (type: FlipConfig['type']) => {
    onUpdate([...flipConfigs, newFlipConfig(type)]);
    toast.success(`Added ${type} flip`);
  };

  const updateFlip = (index: number, config: FlipConfig) => {
    const updated = [...flipConfigs];
    updated[index] = config;
    onUpdate(updated);
  };

  const removeFlip = (index: number) => {
    onUpdate(flipConfigs.filter((_, i) => i !== index));
    toast.info('Flip removed');
  };

  const renderConfig = (config: FlipConfig, onChange: (c: FlipConfig) => void) => {
    switch (config.type) {
      ...
      {flipConfigs.length === 0 ? (
        ...
        <div className="space-y-3">
          {flipConfigs.map((config, index) => {
            ...
            return (
              <motion.div key={index} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }} className="rounded-2xl border border-border/50 bg-card overflow-hidden">
                <div className="flex items-center justify-between p-4 border-b border-border/30">
                  ...
                  <div className="flex items-center gap-2">
                    <Switch checked={config.enabled ?? true} onCheckedChange={v => updateFlip(index, { ...config, enabled: v })} />
                    <Button variant="ghost" size="icon" onClick={() => removeFlip(index)} className="h-8 w-8 text-destructive hover:bg-destructive/10 rounded-xl">
